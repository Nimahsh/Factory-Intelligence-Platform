...
if df_filtered.empty:
    st.warning("No records match the selected filters. Try a wider date range or a different line/shift/product.")
    st.stop()
...
if Selected_line != "All":
    ...
    if Line_SpeedDi.empty:
        st.warning("No positive-production speed records are available for this filter. Stability and stoppage sections are skipped.")
        st.stop()
...
df_stoppage = pd.read_excel("Code_map.xlsx")
stoppage_map = {
    row["Code"]: {
        "Kind": row["Kind"],
        "Machine": row["Machine"],
...
if Selected_line != "All":
    ...
    df_kind = pd.read_excel("Kind_map.xlsx")
    Kind_map = {
        row["Stoppage Kind"]: {
            "Stop": row["Stop"],}
    ...
    Line_Stoppages["Symbol of Stoppages"] = Line_Stoppages["Code"].map(lambda x: stoppage_map.get(x, {}).get("Kind", "Unknown"))
    Line_Stoppages["Reason"] = Line_Stoppages["Code"].map(lambda x: stoppage_map.get(x, {}).get("Machine", "Unknown"))
    Line_Stoppages["Comment"] = Line_Stoppages["Code"].map(lambda x: stoppage_map.get(x, {}).get("Issue", "Unknown"))
    # Line_Stoppages["Type"] = Line_Stoppages["Code"].map(lambda x: stoppage_map[x]["Type"])
    Line_Stoppages["Classification"] = Line_Stoppages["Symbol of Stoppages"].map(lambda x: Kind_map.get(x, {}).get("Stop", "Unknown"))
    ...
    if Line_Stoppages.empty:
        st.info("No mapped stoppage records remain after ignored codes are removed for this filter.")
        st.stop()
    ...
    if total_loss == 0:
        st.info("Stoppage records exist, but total stoppage minutes are zero for this filter.")
        st.stop()
    ...
    #----------------------------------------------------------------
        ...
        if reason_df.empty or reason_df["Minute"].sum() == 0:
            st.info("No reason-level stoppage detail is available for this classification.")
            st.stop()
